...
func (g *Generator) genRoute(rPath string, r *route.Route) error {
	...
	for _, form := range r.Template().GetForms() {
		...
		form.Node.DPCtxFieldName = getRouteDataProviderCtxFormField(form.Name)
	...
	for _, form := range r.Template().GetForms() {
		buf.WriteString(getRouteDataProviderFormInitMethod(rPath, form.Name))
		buf.WriteStringLn("(ctx context.Context, r *mux.Request, w mux.ResponseWriter, data *" + getFormValuesTypeName(form.Name) + ") error")

		buf.WriteString(getRouteDataProviderFormProcessMethod(rPath, form.Name))
		buf.WriteStringLn("(ctx context.Context, r *mux.Request, w mux.ResponseWriter, data *" + getFormValuesTypeName(form.Name) + ") error")
	}
	buf.WriteStringLn("}\n")
	...
	for _, form := range r.Template().GetForms() {
		dCtxFormField := "dataCtx." + getRouteDataProviderCtxFormField(form.Name)
		...
		buf.WriteStringLn("if err := dp." + getRouteDataProviderFormInitMethod(rPath, form.Name) + "(ctx, r, w, &" + dCtxFormField + "); err != nil {")
	...
	if len(r.Template().GetForms()) > 0 {
		...
		for _, form := range r.Template().GetForms() {
			formField := getRouteDataProviderCtxFormField(form.Name)
			...
			buf.WriteStringLn("if err := dp." + getRouteDataProviderFormProcessMethod(rPath, form.Name) + "(ctx, r, w, &dataCtx." +
				formField + "); err != nil {")
			buf.WriteStringLn("	return nil, err")
			buf.WriteStringLn("	}")
		...
		for _, form := range r.Template().GetForms() {
			buf.WriteString(",")
			buf.WriteString("&dataCtx." + getRouteDataProviderCtxFormField(form.Name) + ".BaseFormValues")
		}
	...
	for _, form := range r.Template().GetForms() {
		buf.WriteStringLn(getRouteDataProviderCtxFormField(form.Name) + " " + getFormValuesTypeName(form.Name))
	}
	buf.WriteStringLn("}")
...
func getRouteDataProviderFormInitMethod(rPath string, formName string) string {
	return "Init" + strings.TrimPrefix(getRouteDataProviderMethod(rPath), "Get") + "_Form" + getExportedName(formName)
}

func getRouteDataProviderFormProcessMethod(rPath string, formName string) string {
	return "Process" + strings.TrimPrefix(getRouteDataProviderMethod(rPath), "Get") + "_Form" + getExportedName(formName)
}

func getRouteDataProviderCtxFormField(formName string) string {
